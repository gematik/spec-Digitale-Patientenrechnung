...
Der FD bringt auf jedem verarbeiteten Dokument einen Datamatrix-Code (Token-Barcode) an einer Default-Position an. Für jede Rechnung und jeden Anhang KANN diese Position über den Part `qrPosition` übersteuert werden. Er enthält die Sub-Parts `x` und `y` mit den Zielkoordinaten als `decimal` in `pt` (typografischer Punkt). Wird `qrPosition` angegeben, MÜSSEN `x` und `y` gemeinsam übermittelt werden. Ohne `qrPosition` gilt die Default-Position.
